...
// GetAllReservations return slice of all reservations.
func (m *postgresDBRepo) GetAllReservations() ([]models.Reservation, error) {
	...
	query := `SELECT r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date,
			  ...
			  FROM reservations r JOIN rooms rm
			  ON r.room_id = rm.id ORDER BY r.start_date asc`
	...
	for rows.Next() {
		...
		err := rows.Scan(&res.ID, &res.FirstName, &res.LastName, &res.Email, &res.Phone, &res.StartDate,
			&res.EndDate, &res.RoomID, &res.CreatedAt, &res.UpdatedAt, &res.Room.ID, &res.Room.RoomName)
		if err != nil {
			return reservations, err
		}
...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	err := row.Scan(&res.ID, &res.FirstName, &res.LastName, &res.Email, &res.Phone, &res.StartDate,
		&res.EndDate, &res.RoomID, &res.CreatedAt, &res.UpdatedAt, &res.Processed, &res.Room.ID, &res.Room.RoomName)
	if err != nil {
		return res, err
	}
...
func (m *postgresDBRepo) UpdateReservation(updatedRes models.Reservation) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	_, err := m.DB.ExecContext(ctx, query, updatedRes.FirstName, updatedRes.LastName, updatedRes.Email,
		updatedRes.Phone, time.Now(), updatedRes.ID)
	if err != nil {
		return err
	}
...
func (m *postgresDBRepo) UpdateProcessed(id, proccess int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	query := `UPDATE reservations SET processed = $1 WHERE id = $2`
	_, err := m.DB.ExecContext(ctx, query, proccess, id)
	if err != nil {
		return err
	}
